...
def test_check_password_strength_very_weak_short():
    ...
    # Or masked if you implement that
    # TODO: Assert that `data["strength"]`
    # is "very_weak" or similar based on your logic.
    ...
    assert data["score"] <= 1
    assert "Password should be at least "
    "8 characters long." in data["suggestions"]
...
def test_check_password_strength_medium_only_lowercase_and_long():
    ...
    # TODO: Assert the strength category
    # (e.g., "medium" or "weak" depending on scoring).
    # TODO: Assert that suggestions include adding uppercase,
    # digits, and symbols.
    # Example:
    assert data["strength"] in ["weak", "medium", "average"]
...
def test_check_password_strength_strong_all_criteria():
    ...
    # Example:
    assert data["strength"] in ["ok", "good", "strong"]
    assert data["score"] >= 4
